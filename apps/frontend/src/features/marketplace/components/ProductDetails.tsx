...
export const ProductDetails = ({ product, onOrderClick }: ProductDetailsProps) => {
  ...
  return (
    <div className="grid grid-cols-1 gap-12 lg:grid-cols-2">
      ...
      <div className="flex flex-col pt-4">
        ...
        <div className="mb-8 text-3xl font-semibold text-neutral-900 dark:text-neutral-50">
          \${(product.price / 100).toFixed(2)}
        </div>

        {product.seller && (
          <div className="mb-8 flex items-center gap-3 rounded-xl border border-neutral-200 p-4 dark:border-neutral-800">
            <div className="h-10 w-10 overflow-hidden rounded-full bg-neutral-100 dark:bg-neutral-800">
              {product.seller.image && (
                <img src={product.seller.image} alt="" className="h-full w-full object-cover" />
              )}
            </div>
            <div>
              <p className="text-sm text-neutral-500">Created by</p>
              <p className="font-medium text-neutral-900 dark:text-neutral-50">@{product.seller.username}</p>
            </div>
          </div>
        )}

        <div 
          className="prose prose-neutral dark:prose-invert mb-10 max-w-none"
          dangerouslySetInnerHTML={{ __html: product.description }}
        />

        <div className="mt-auto flex flex-col gap-4 border-t border-neutral-200 pt-8 dark:border-neutral-800">
